    ...
    await __open(async (db) => {
        ...
        await db.run(`
            CREATE TABLE IF NOT EXISTS Transactions (
                ...
                productId INTEGER REFERENCES Racks(id),
...
export const addTransaction = async (transaction: { productId: number, quantity: number, total: number }): Promise<void> => {
    await __open(async (db) => {
        await db.run(
            "INSERT INTO Transactions (productId, quantity, total, status) VALUES (?, ?, ?, ?)",
...
export const updateTransactionStatus = async (transactionId: number, status: string): Promise<void> => {
    await __open(async (db) => {
        await db.run("UPDATE Transactions SET status = ? WHERE transactionId = ?", status, transactionId);
    });
};

export const getTransactions = async (status?: string): Promise<any[]> => {
    return await __open(async (db) => {
        if (status) {
            return await db.all("SELECT * FROM Transactions WHERE status = ?", status);
        }
...
const db = { reset, getRacks, getCategories, getRacksByCategory, addProduct, updateProduct,addTransaction,updateTransactionStatus,getTransactions, exists };
export default db;
